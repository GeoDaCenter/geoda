...
bool TreeCell::containsPoint(double point[])
{   
...
{   
    ...
    for (int n = 0; n < N; n++) {
        for (int d = 0; d < QT_NO_DIMS; d++) {
            ...
            min_Y[d] = mind(min_Y[d], inp_data[n * QT_NO_DIMS + d]);
            max_Y[d] = maxd(max_Y[d], inp_data[n * QT_NO_DIMS + d]);
        }
    ...
    for (int d = 0; d < QT_NO_DIMS; d++) {
        mean_Y[d] /= (double) N;
        width_Y[d] = maxd(max_Y[d] - mean_Y[d], mean_Y[d] - min_Y[d]) + 1e-5;    
    }
