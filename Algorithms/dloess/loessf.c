/* loessf.f -- translated by f2c (version 20160102).
...
/* Computing MIN */
    ...
    *rcond = min(d__1,d__2);
	...
	++(*sing);
	if (*sing == 1) {
	    ehg184_("Warning. pseudoinverse used at", &q[1], d__, &c__1, (
		    ftnlen)30);
	    d__1 = sqrt(rho);
	...
	delta1, doublereal *delta2)
{
    ...
    /* Local variables */
    static integer i__;
    static doublereal z__, c1, c2, c3, c4, corx;
...
doublereal dsign_(doublereal *a1, doublereal *a2)
{
    /* System generated locals */
    doublereal ret_val;
...
/* Subroutine */ int ehg139_(doublereal *v, integer *nvmax, integer *nv, 
	...
	trl, integer *kernel, integer *k, doublereal *dist, integer *phi, 
	doublereal *eta, doublereal *b, integer *od, doublereal *w, 
...
/* L11: */
	    ...
	    ehg137_(z__, &vhit[l], leaf, &nleaf, d__, nv, nvmax, ncmax, vc, &
		    a[1], &xi[1], &lo[1], &hi[1], &c__[c_offset], &v[v_offset]
		    );
	    i__2 = nleaf;
...
L7:
	if (m + offset >= u || m + offset < l) {
	    ...
	    ehg106_(&lower, &upper, &check, &c__1, &x[k * x_dim1 + 1], &pi[1],
		     n);
...
/* Subroutine */ int ehg137_(doublereal *z__, integer *kappa, integer *leaf, 
	...
	ncmax, integer *vc, integer *a, doublereal *xi, integer *lo, integer *
	hi, integer *c__, doublereal *v)
{
    /* Initialized data */
