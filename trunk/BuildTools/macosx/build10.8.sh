...
install_library freexl-1.0.0f http://www.gaia-gis.it/gaia-sins/freexl-sources/freexl-1.0.0f.tar.gz libfreexl.a
{
    ...
    LIB_URL="http://www.gaia-gis.it/gaia-sins/freexl-1.0.0f.tar.gz"
    LIB_CHECKER="libfreexl.a"
    LIB_FILENAME="freexl-1.0.0f.tar.gz"
    echo $LIB_FILENAME
    ...
    if ! [ -f "$PREFIX/lib/$LIB_CHECKER" ] ; then
        cd $LIB_NAME
    chmod +x configure
        ./configure -L./libraries/lib -liconv --prefix=$PREFIX
        $MAKER
        make install
    fi  
    if ! [ -f "$PREFIX/lib/$LIB_CHECKER" ] ; then
        echo "For libiconv error: modify Makefile src/Makefile to setup -L/usr/lib -liconv"
        echo "Error! Exit"
        exit
    fi  
}
...
if ! [ -f "$PREFIX/lib/$LIB_CHECKER" ] ; then
    echo "For OSX 10.9, comment out line 161 and try to replace glibtoolize with libtoolize."
    echo "Error! Exit"
    exit
fi
#########################################################################
...
if ! [ -f "$PREFIX/lib/$LIB_CHECKER" ] ; then
    cd $LIB_NAME
    ./configure CC="$GDA_CC" CFLAGS="$GDA_CFLAGS -I$PREFIX/include" CXX="$GDA_CXX" CXXFLAGS="$GDA_CXXFLAGS -I$PREFIX/include" LDFLAGS="$GDA_LDFLAGS -L$PREFIX/lib -lgeos_c -liconv" --prefix=$PREFIX --enable-iconv --enable-proj --enable-geos --with-geosconfig=$PREFIX/bin/geos-config
    $MAKER
    touch src/.libs/libspatialite.lai
    make install
    cp src/.libs/* $GEODA_HOME/libraries/lib
fi
...
if ! [ -f $DOWNLOAD_HOME/$LIB_NAME/stage/lib/$LIB_CHECKER ]; then
    ...
    ./b2 link=static threading=multi cxxflags=$GDA_CXXFLAGS stage
fi
...
LIB_FILENAME=gdal-1.9.2.tar.gz
...
if ! [ -f "$PREFIX/lib/$LIB_CHECKER" ] ; then
    ...
    #cp -rf $GEODA_HOME/dep/$LIB_NAME/* .
    sed -i '' "s/gcc-4.2/gcc/g" GDALmake.opt
    sed -i '' "s/g++-4.2/gcc/g" GDALmake.opt
    #make clean
    $MAKER
...
#########################################################################
#LIB_NAME=wxWidgets-2.9.5
LIB_NAME=wxWidgets-3.0.0
LIB_URL=http://softlayer-dal.dl.sourceforge.net/project/wxwindows/3.0.0/wxWidgets-3.0.0.tar.bz2
#LIB_URL=http://softlayer-dal.dl.sourceforge.net/project/wxwindows/2.9.5/wxWidgets-2.9.5.tar.bz2
LIB_FILENAME=$(basename "$LIB_URL" ".tar")
...
if ! [ -f "$PREFIX/lib/$LIB_CHECKER" ] ; then
    ...
    #./configure CFLAGS="$GDA_CFLAGS" CXXFLAGS="$GDA_CXXFLAGS" LDFLAGS="$GDA_LDFLAGS" OBJCFLAGS="" OBJCXXFLAGS="" --with-cocoa --disable-shared --disable-monolithic --with-opengl --enable-postscript --with-macosx-version-min=10.6 --enable-textfile --prefix=$PREFIX
    $MAKER 
    make install
    cd ..
fi
