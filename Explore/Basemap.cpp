...
Basemap::Basemap(BasemapItem& _basemap_item, Screen* _screen, MapLayer* _map, MapLayer* _origMap, wxString _cachePath,
                 OGRCoordinateTransformation* _poCT, double _scale_factor) {
...
void Basemap::SetupMapType(BasemapItem& _basemap_item) {
  ...
  // get a latest Stadia key
  std::vector<wxString> stadia_user = OGRDataAdapter::GetInstance().GetHistory("stadia_key");
  if (!stadia_user.empty()) {
    wxString key = stadia_user[0];
    if (!key.empty()) {
      stadia_key = key;
    }
  }
...
void Basemap::Extent(double _n, double _w, double _s, double _e, OGRCoordinateTransformation* _poCT) {
  if (poCT != NULL) {
...
bool Basemap::Zoom(bool is_zoomin, int x0, int y0, int x1, int y1) {
  if (is_zoomin == false && zoom <= 1) return false;
...
bool Basemap::IsExtentChanged() {
  bool no_change = (origMap->north == map->north && origMap->south == map->south && origMap->east == map->east &&
...
int Basemap::GetEasyZoomLevel() {
  ...
  double zoomH = static_cast<int>(ceil(log2(degreeRatio * screen->width / 256.0)));

  degreeRatio = 85.0511 * 2.0 / map->GetHeight();
  double zoomV = static_cast<int>(ceil(log2(degreeRatio * screen->height / 256.0)));

  if (zoomH > 0 && zoomV > 0) {
    zoom = std::min(zoomH, zoomV);
  } else {
    if (zoomH > 0) zoom = zoomH;
    if (zoomV > 0) zoom = zoomV;
  }
...
void Basemap::GetTiles() {
  ...
  if (widthP < screen->width) {
    int x_addition = static_cast<int>(ceil((screen->width - widthP) / 256.0));
    endX += x_addition;
    widthP = (endX - startX + 1) * 256;
  }
  if (heightP < screen->height) {
    int y_addition = static_cast<int>(ceil((screen->height - heightP) / 256.0));
    endY += y_addition;
    heightP = (endY - startY + 1) * 256;
  }
  ...
  int map_offx = static_cast<int>((ceil)((screen->width - map_wp) / 2.0));
  ...
  int map_offy = static_cast<int>(((screen->height - map_hp) / 2.0));
...
bool Basemap::IsDownloading() { return start_download; }
...
XYFraction* Basemap::LatLngToXY(LatLng& latlng) {
  ...
  int xp = static_cast<int>(x * 256 - leftP) - offsetX;
  int yp = static_cast<int>(y * 256 - topP) - offsetY;
...
void Basemap::LatLngToXY(double lng, double lat, int& x, int& y) {
  ...
  y = static_cast<int>(yy * 256 - topP) - offsetY;

  double xx = (lng + 180.0) / 360.0 * nn;
  x = static_cast<int>(xx * 256 - leftP) - offsetX;
}
...
wxString Basemap::GetRandomSubdomain(wxString url) {
  ...
  if (url.Find("openstreetmap") != wxNOT_FOUND || url.Find("carto") != wxNOT_FOUND ||
      url.Find("wmflabs") != wxNOT_FOUND || url.Find("fastly") != wxNOT_FOUND) {
    domains.push_back("a");
    domains.push_back("b");
    domains.push_back("c");
  } else if (url.Find("here") != wxNOT_FOUND || url.Find("bdimg") != wxNOT_FOUND) {
    domains.push_back("1");
    domains.push_back("2");
    domains.push_back("3");
    domains.push_back("4");
...
wxString Basemap::GetTileUrl(int x, int y) {
  ...
  url.Replace("STADIA_KEY", stadia_key);
...
bool Basemap::Draw(wxBitmap* buffer) {
  ...
  for (int i = x0; i <= x1; i++) {
    for (int j = startY; j <= endY; j++) {
      ...
      } else if (imageSuffix.CmpNoCase(".jpeg") == 0 || imageSuffix.CmpNoCase(".jpg") == 0) {
