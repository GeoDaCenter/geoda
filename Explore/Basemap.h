...
class BasemapItem {
 public:
  ...
  bool operator==(const BasemapItem& other) { return (group == other.group && name == other.name && url == other.url); }
...
class BasemapGroup {
 public:
  BasemapGroup() {}
  explicit BasemapGroup(wxString _name) { name = _name; }
  ~BasemapGroup() {}
  ...
  void AddItem(BasemapItem item) { items.push_back(item); }
  wxString name;
...
class XYFraction {
 public:
  ...
  int GetXInt() { return static_cast<int>(xint); }
  int GetYInt() { return static_cast<int>(yint); }
...
class MapLayer {
 public:
  ...
  explicit MapLayer(MapLayer* _map) {
    north = _map->north;
    south = _map->south;
    west = _map->west;
    east = _map->east;
  ...
  MapLayer(double _n, double _w, double _s, double _e, OGRCoordinateTransformation* _poCT) {
    north = _n;
    south = _s;
    west = _w;
    east = _e;
...
class Basemap {
 ...
 public:
  Basemap() {}
  Basemap(BasemapItem& basemap_item, Screen* _screen, MapLayer* _map, MapLayer* _origMap, wxString _cachePath,
          OGRCoordinateTransformation* _poCT, double scale_factor = 1.0);
  ~Basemap();
  ...
  void Extent(double _n, double _w, double _s, double _e, OGRCoordinateTransformation* _poCT);
