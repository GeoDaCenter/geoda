...
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
    sudo apt install cmake curl dh-autoreconf libexpat1-dev libreadline-dev zlib1g-dev libgtk-3-dev libgl1-mesa-dev libglu1-mesa-dev libwebkit2gtk-4.0-dev libssl-dev
fi
...
MAKER="make -j $CPUS"
export CFLAGS="-m64 -std=c++98"
export CXXFLAGS="-m64 -std=c++98"
...
# sudo apt-get install libgtk2.0-dev libglu1-mesa-dev libgl1-mesa-dev
{
    ...
    cd $DOWNLOAD_HOME
    if ! [ -f "$LIB_FILENAME" ] ; then
        git clone --recurse-submodules https://github.com/wxWidgets/wxWidgets.git wxWidgets-3.1.2
    fi
    ...
    if ! [ -f "$PREFIX/bin/$LIB_CHECKER" ] ; then
        ...
        ./configure --with-gtk=3 --disable-shared --disable-monolithic --enable-streams --with-libpng --enable-image --enable-svg --with-opengl --enable-postscript --without-libtiff --disable-debug --enable-webview --prefix=$PREFIX
        $MAKER
        make install
        cd ..
    fi
...
{
    cd $GEODA_HOME
    cp ../../GeoDamake.disco.opt ../../GeoDamake.opt
    mkdir ../../o
    $MAKER
    make app
}
